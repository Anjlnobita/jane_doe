import threading
from jane.database import client, noxarion
...
def set_flood(chat_id, amount):
    with INSERTION_FLOOD_LOCK:
        flood = noxarion.find_one({"chat_id": str(chat_id)})
        if not flood:
            flood = {"chat_id": str(chat_id), "user_id": None, "count": DEF_COUNT, "limit": amount}
            noxarion.insert_one(flood)
        else:
            noxarion.update_one({"chat_id": str(chat_id)}, {"$set": {"limit": amount}})
...
def set_flood_strength(chat_id, flood_type, value):
    with INSERTION_FLOOD_SETTINGS_LOCK:
        curr_setting = noxarion.find_one({"chat_id": str(chat_id)})
        if not curr_setting:
            curr_setting = {"chat_id": str(chat_id), "flood_type": int(flood_type), "value": str(value)}
            noxarion.insert_one(curr_setting)
        else:
            noxarion.update_one({"chat_id": str(chat_id)}, {"$set": {"flood_type": int(flood_type), "value": str(value)}})

def get_flood_setting(chat_id):
    setting = noxarion.find_one({"chat_id": str(chat_id)})
    if setting:
...
def migrate_chat(old_chat_id, new_chat_id):
    with INSERTION_FLOOD_LOCK:
        flood = noxarion.find_one({"chat_id": str(old_chat_id)})
        if flood:
            CHAT_FLOOD[str(new_chat_id)] = CHAT_FLOOD.get(str(old_chat_id), DEF_OBJ)
            noxarion.update_one({"chat_id": str(old_chat_id)}, {"$set": {"chat_id": str(new_chat_id)}})

def __load_flood_settings():
    global CHAT_FLOOD
    all_chats = noxarion.find()
